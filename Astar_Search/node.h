...
public:
	...
	/* 标识（只含有token的place） */
	vector<Place> state_;
	Node(NodePool* pool) :discarded_(false), is_deadlock_(false), is_open_(false), g_(0), h_(0), id_(Node::num), pool_(pool) { ++Node::num; }
		...
		while (i <= state_.back().row_) {
			if (i < state_[j].row_) {
				if (v[i] > 0) {
					...
					newnode->state_.emplace_back(Place(v[i], i, { 0, 0 }));
				}
			}
			else {
	...
	Place& operator[](int i) {
		Place temp(0, -1, vector<int16_t>(2, 0));
		for (auto& p : state_) {
			if (p.row_ < i)
				continue;
			else if (p.row_ == i)
